...
def build_pdf():
    ...
    story.append(Paragraph(
        "<b>Auteur</b> : Philippe KEVORKIAN", ctx_style))
    story.append(Paragraph(
    ...
    comp_data = [
        [Paragraph('<b>CONFIGURATION</b>', ParagraphStyle('', fontSize=8, textColor=WHITE, fontName='Helvetica-Bold', alignment=TA_CENTER)),
         Paragraph('<b>LEMOIGNE</b>', ParagraphStyle('', fontSize=8, textColor=WHITE, fontName='Helvetica-Bold', alignment=TA_CENTER)),
         Paragraph('<b>GRELET</b>', ParagraphStyle('', fontSize=8, textColor=WHITE, fontName='Helvetica-Bold', alignment=TA_CENTER)),
         Paragraph('<b>DISTANCE À 50%</b>', ParagraphStyle('', fontSize=8, textColor=WHITE, fontName='Helvetica-Bold', alignment=TA_CENTER))],
        [Paragraph('Duel (hypothétique)', cell_style),
         ...
         Paragraph('Grelet à 2,8 pts de 50%', cell_style)],
        [Paragraph('<b>Triangulaire (réelle)</b>', ParagraphStyle('', fontSize=8, fontName='Helvetica-Bold', textColor=GREY_DARK, alignment=TA_CENTER)),
         ...
         Paragraph('<b>Grelet à 8,6 pts de 50%</b>', ParagraphStyle('', fontSize=8, fontName='Helvetica-Bold', textColor=GREEN, alignment=TA_CENTER))],
    ]
    comp_table = Table(comp_data, colWidths=[40*mm, 30*mm, 30*mm, 50*mm])
    ...
    story.append(Paragraph("5.4 Fait avéré : attaque de la permanence Lemoigne", styles['h2']))

    story.append(Paragraph(
        "Dans la nuit du vendredi 20 mars 2026, la permanence de campagne de Romain Lemoigne "
        "a été caillassée par un groupe d'individus identifiés comme appartenant à la mouvance "
        "antifasciste, venus de l'extérieur de la commune. Cet acte de violence politique, "
        "survenant à J-2 du scrutin, constitue une tentative d'intimidation caractérisée.",
        styles['body']))

    story.append(Paragraph(
        "L'impact électoral de ce type d'action est double :",
        styles['body']))

    intim_items = [
        "<b>Effet démobilisateur</b> : les sympathisants Lemoigne les moins engagés peuvent "
        "être découragés par le climat de violence, réduisant la participation dans leur camp "
        "(estimé : 20 à 80 voix de démobilisation différentielle).",
        "<b>Effet de solidarité inverse</b> : paradoxalement, une agression peut aussi galvaniser "
        "la base électorale de la victime (« effet martyr »). Dans les communes rurales et "
        "périurbaines, ce type de violence est généralement rejeté par l'ensemble du corps électoral."
    ]
    for item in intim_items:
        num_style = ParagraphStyle('Num', parent=styles['body'],
            leftIndent=8*mm, bulletIndent=0, spaceBefore=1*mm)
        story.append(Paragraph(f"<font color='#E53935'>&#8226;</font> {item}", num_style))

    story.append(Paragraph("5.5 Recommandations de vigilance", styles['h2']))
    ...
    risk_text = (
        ...
        "physique ont déjà été constatés (attaque de la permanence Lemoigne le 20 mars). "
    ...
    # ══════════════════════════════════════════════
    # PAGE 7 : ANNEXE TECHNIQUE
    # ══════════════════════════════════════════════

    story.append(ColorBar(page_w, 2*mm, GREY_MED))
    story.append(Spacer(1, 2*mm))
    story.append(Paragraph("Annexe technique", styles['h1']))

    story.append(Paragraph("A.1 Données sources", styles['h2']))
    story.append(Paragraph(
        "Les données électorales proviennent de DataRealis (fichier 72154) pour les scrutins "
        "nationaux et du Ministère de l'Intérieur pour les municipales 2026. Les résultats "
        "sont disponibles au niveau de chacun des 17 bureaux de vote de la commune.",
        styles['small']))
    ...
    story.append(Paragraph("A.2 Modèle de projection", styles['h2']))
    story.append(Paragraph(
        "<b>Étape 1 — Inférence écologique :</b> Corrélations de Pearson et régressions linéaires "
        "bivariées entre scores nationaux (variable explicative) et scores municipaux (variable à "
        "expliquer) sur les 17 bureaux de vote. Meilleur prédicteur unique : Macron législatives 2024 "
        "→ Da Silva municipales 2026 (R<super>2</super> = 0,548).",
        styles['small']))
    story.append(Paragraph(
        "<b>Étape 2 — Matrices de reports :</b> Construction de matrices de transfert "
        "(blocs nationaux → listes municipales) calibrées sur les corrélations écologiques et "
        "les hypothèses qualitatives. Chaque cellule de la matrice est perturbée de manière aléatoire "
        "via des lois de Dirichlet (paramètres alpha calibrés sur la moyenne et l'écart-type de "
        "chaque ligne).",
        styles['small']))
    story.append(Paragraph(
        f"<b>Étape 3 — Simulation stochastique :</b> {NB_TIRAGES_TXT} tirages indépendants dans les "
        "distributions de Dirichlet. Pour chaque tirage, calcul du résultat électoral projeté. "
        "Perturbation additionnelle de la participation (+/- 2 points). Comptage des victoires "
        "par candidat = probabilité de victoire.",
        styles['small']))
    story.append(Paragraph(
        "<b>Étape 4 — Mise à jour bayésienne :</b> Lorsque des résultats réels de bureaux "
        "sont disponibles (soirée électorale), le modèle se met à jour par échantillonnage pondéré "
        "(vraisemblance gaussienne, rééchantillonnage systématique si la taille effective "
        "de l'échantillon descend sous N/4).",
        styles['small']))

    story.append(Paragraph("A.3 Hypothèses centrales du scénario T2", styles['h2']))
    ...
    story.append(Paragraph("A.4 Résultats T1 de référence", styles['h2']))
    ...
    t1_data = [
        ...
        [Paragraph('Romain Lemoigne (RN)', ParagraphStyle('', fontSize=8, textColor=COL_LEMOIGNE, fontName='Helvetica-Bold')),
         Paragraph('3 014', cell_style), Paragraph('43,68%', cell_style), Paragraph('29,18%', cell_style)],
    ...
    t1_table = Table(t1_data, colWidths=[55*mm, 25*mm, 25*mm, 25*mm])
    ...
    story.append(Spacer(1, 3*mm))
    story.append(Paragraph(
        "Inscrits : 10 330 | Votants : 7 114 (68,87%) | Blancs : 130 | Nuls : 84 | Exprimés : 6 900",
        styles['caption']))
